...
public class Gem3DRenderer {
    ...
    public static void initialize() {
        ...
        try {
            ...
            // Прозрачност: само формата/контурът на камъка се вижда, без оцветяване около нея.
            Material material = new Material(
                    TextureAttribute.createDiffuse(blockTexture),
                    ColorAttribute.createDiffuse(Color.WHITE),
                    ColorAttribute.createSpecular(0.4f, 0.4f, 0.5f, 1f),
    ...
    public static void addGem(float centerX, float centerY, float size, Color baseColor) {
        ...
        // Позиция в центъра на клетката (z = 0)
        instance.transform.idt();
        instance.transform.setToTranslation(centerX, centerY, 0f);

        // Мащаб по клетката
        float uniformScale = size * 0.9f;
        instance.transform.scale(uniformScale, uniformScale, uniformScale); // равномерно за истински 3D обем

        // Лек наклон за 3D вид – предната страна (block.png) остава добре видима
        instance.transform.rotate(Vector3.X, 22f);

        // Въртене около вертикалната ос (Y) – като истински 3D обект: от ляво надясно,
        // виждат се предната и страничните стени (всички с block.png текстура)
        instance.transform.rotate(Vector3.Y, rotationAngleDeg);

        // Per-instance color via material.
        Material mat = instance.materials.first();
        ColorAttribute diffuse = (ColorAttribute) mat.get(ColorAttribute.Diffuse);
        if (diffuse != null) {
            diffuse.color.set(baseColor);
        }
    ...
    public static void renderAll() {
        if (!initialized || modelBatch == null || camera == null || instances.size == 0) return;
